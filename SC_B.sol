...
contract SC_B {
    event Deposit(bytes32 indexed TrSC_A, bytes32 indexed Trpr, bytes32 S, uint256 timestamp);
    ...
    function AuditConsolidation(bytes32 TrSC_A, bytes32 Trpr, bytes32 S1, bytes32 S2, bytes32 psi) public {
        bytes32 rawS = keccak256(abi.encodePacked(S1, S2, psi));
        uint256 S = uint256(rawS) % P;
        if (S == 0) {
            S = 1;
        }
        emit Deposit(TrSC_A, Trpr, bytes32(S), block.timestamp);
